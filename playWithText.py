...
class PlayWithText:
    ...
    def saveFile(self):
        global file
        if not file:  # if no file selected yet
            if len(userTextResult.get(1.0, END).strip()) > 0:
                file = asksaveasfilename(
                    initialfile="Untitled.txt",
                    defaultextension=".txt",
                    filetypes=[("Text Documents", "*.txt"), ("All Files", "*.*")],
                )
                if file:  # user selected a path
                    with open(file, "w") as f:
                        f.write(userTextResult.get(1.0, END))
                    root.title(os.path.basename(file) + " - Notepad")
            else:
                msg = tmsg.askokcancel(
                    title="Actions Box Was Empty",
                    message="Action Box is empty. Do you want to save your original content?",
                )
                if msg:
                    file = asksaveasfilename(
                        initialfile="Untitled.txt",
...
if __name__ == "__main__":
    ...
    heading = Label(
        root,
        text="Welcome to Play With Text By Manisha",
        font="lucida 15 bold",
        fg="#3e1d7e",
    )
    heading.pack(pady=5)
